...
class shared_depthwise_conv2d(object):
    ...
    def __init__(self, input, weight_size, strides=[1, 1, 1, 1], padding='SAME', pool='p', pool_size=4,
        ...
        with tf.variable_scope(name):
            self.weight = tf.Variable(tf.tile(
                tf.reduce_mean(tf.random_normal(weight_size, stddev=std, dtype=tf.float32), axis=2, keep_dims=True),
                [1, 1, weight_size[2], 1]))
            self.bias = tf.Variable(tf.random_normal([weight_size[-1]], stddev=std, dtype=tf.float32))
    ...
    def __init__(self, input, weight_size, strides=[1, 1, 1, 1], padding='SAME', pool='p', pool_size=4,
        ...
        with tf.variable_scope(name):
            ...
            network = tf.nn.bias_add(
                tf.nn.depthwise_conv2d(input=input, filter=self.weight, strides=strides, padding=padding),
                self.bias, name=name)
            if use_batchnorm:
                batch_mean, batch_var = tf.nn.moments(network, axes=[0])
                network = tf.nn.batch_normalization(network, batch_mean, batch_var, offset=offset, scale=scale,
...
class RCL_coef(object):
    def __init__(self, input, weight_size, strides=[1, 1, 1, 1], padding='SAME', pool='p', pool_size=[1, 4], num_iter=3,
        ...
        with tf.variable_scope(name):
            ...
            self.coef_weight = [
                tf.Variable(tf.random_normal([1, 1, weight_size[-2], weight_size[-1]], stddev=std, dtype=tf.float32))
                for i in range(num_iter)]
            self.coef_bias = [tf.Variable(tf.random_normal([weight_size[-1]], stddev=std, dtype=tf.float32)) for i in
                              range(num_iter)]
            """
            ...
            if num_iter == 0:
                ...
                for i in range(num_iter):
                    ...
                    network = tf.nn.bias_add(
                        tf.nn.conv2d(input=network, filter=self.coef_weight[i], strides=strides, padding=padding),
                        self.coef_bias[i]
                        )
                    if use_batchnorm:
                        batch_mean, batch_var = tf.nn.moments(network, [0])  # [0,1,2]
...
class depthwise_RCL(object):
    def __init__(self, input, weight_size, strides=[1, 1, 1, 1], padding='SAME', pool='p', pool_size=[1, 4], num_iter=3,
        ...
        with tf.variable_scope(name):
            ...
            # self.bias = tf.Variable(tf.random_normal([weight_size[-1]*weight_size[-2]], stddev=std, dtype=tf.float32))
            self.biases = [
                tf.Variable(tf.random_normal([weight_size[-1] * weight_size[-2]], stddev=std, dtype=tf.float32)) for i \
                in range(num_iter + 1)]
            """
            ...
            network = input
            network = tf.nn.bias_add(
                tf.nn.depthwise_conv2d(input=network, filter=self.weight, strides=strides, padding=padding),
                self.biases[0])
            if use_batchnorm:
                batch_mean, batch_var = tf.nn.moments(network, [0])  # [0,1,2]
            ...
            if pool == 'c':
                network = conv2d(input=network,
                                 weight_size=[1, pool_size, weight_size[-1] * weight_size[-2],
    ...
    def __init__(self, input, weight_size, weight=None, bias=None, nonlinearity=None, use_dropout=False, keep_prob=1.0,
        ...
        with tf.variable_scope(name):
            ...
            self.bias = tf.Variable(
                tf.random_normal([weight_size[-1]], stddev=std, dtype=tf.float32)) if bias is None else bias
...
class RCNN(object):
    def __init__(self, batch_size=128, time_point=1024, in_channels=126, out_channels=256, ch_multiplier=None,
                 ...
                 std=0.01, w_filter_size=9, p_filter_size=4, l_rate=0.01, l_decay=0.95, l_step=1000, decay=0.9,
                 momentum=0.9, optimizer='RMSProp', opt_epsilon=0.1):
...
class LSTM(object):
    def __init__(self, std=0.01, batch_size=64, lstm_time=100, lstm_layers=[442, 442, 442], layers=[442, 200, 50, 3],
                 num_sensors=442,
        ...
        # Define loss and optimizer, minimize the squared error
        self.cost = tf.reduce_mean(tf.pow(self.y - self.output, 2))
        self.optimizer = tf.train.RMSPropOptimizer(self.l_rate, decay=self.decay, momentum=self.momentum).minimize(
        ...
        for i in range(len(self.lstm_layers) - 1):
            with tf.variable_scope('lstm' + str(i)):
                cells = tf.nn.rnn_cell.DropoutWrapper(
                    tf.nn.rnn_cell.BasicLSTMCell(self.lstm_layers[i + 1], state_is_tuple=True),
                    self.keep_probs[i])
